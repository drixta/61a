...
def analyze_tweet_sentiment(tweet):
    ...
    False
    >>> love = make_tweet('about about about hate hate hate.',None, 0,0)
    >>> has_sentiment(sentiment_value(analyze_tweet_sentiment(love)))
    True
    """
    ...
    for word in tweet_words(tweet):
        if has_sentiment(get_word_sentiment(word)):
            ...
            count += 1
    if count != 0:
        average = make_sentiment(total_sentiment/count)
    return average
...
def find_centroid(polygon):
    ...
    (1, 2, 0)
    """
    k,area,cx,cy = 0,0,0,0
    while k < len(polygon)-1:
        area += polygon[k][0]*polygon[k+1][1] - polygon[k+1][0]*polygon[k][1]
        cx += (polygon[k][0]+polygon[k+1][0])*(polygon[k][0]*polygon[k+1][1] - polygon[k+1][0]*polygon[k][1])
        cy += (polygon[k][1]+polygon[k+1][1])*(polygon[k][0]*polygon[k+1][1] - polygon[k+1][0]*polygon[k][1])
        k += 1
    if area == 0:
        cx, cy = polygon[0][0],polygon[0][1]
    else:
        area = ((1/2)*area)
        cx,cy = (1/(6*a))*cx,(1/(6*area))*cy
...
def most_talkative_state(term):
    ...
    a = group_tweets_by_state(tweets)
    for state in a:
        if len(a[state]) > high:
            high = len(a[state])
...
def average_sentiments(tweets_by_state):
    ...
    for state in tweets_by_state:
        average, total_sentiment, counter = None, 0,0
        tweets = tweets_by_state[state]
        for tweet in tweets:
            tweet_sentiment = analyze_tweet_sentiment(tweet)
            if has_sentiment(tweet_sentiment):
                total_sentiment += sentiment_value(tweet_sentiment)
                counter += 1
        if counter != 0:
            average = total_sentiment / counter
        if average != None:
...
*    The keys of the returned dictionary are the integers 0 through 23.
    ...
    tweets_by_hour = {}
    for i in tweets:
        if i['time'].hour not in tweets_by_hour:
            tweets_by_hour[i['time'].hour] = []    
        tweets_by_hour[i['time'].hour].append(i) 
